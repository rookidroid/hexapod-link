...
# 5. Server-side: keep the global "run on robot" selection in step
# Running the motion being previewed is the common case, so choosing one here
# arms the global panel's Run button with it. Nothing is sent to the robot until
# that button is pressed.
@app.callback(
    Output(ROBOT_MOTION_SELECT_ID, "value"),
    Input(MOTION_DROPDOWN_ID, "value"),
    prevent_initial_call=True,
)
def sync_robot_motion_selection(motion_name):
    if not motion_name:
        raise PreventUpdate
    return motion_name
